...
function grantNewUserBonus(user) {
  ...
  user.balance = roundMoney(Number(user.balance || 0) + bonus);
  user.bonusGranted = true;
  user.ledger = Array.isArray(user.ledger) ? user.ledger : [];
  user.ledger.unshift(newLedger('new_user_bonus', bonus, user.balance, '新用户注册立减'));
...
function publicUser(user) {
  ...
  return {
    ...
    lastLoginAt: user.lastLoginAt || '', ledger: Array.isArray(user.ledger) ? user.ledger.slice(0, 20) : []
  };
}
...
module.exports = {
  ...
  hashPassword, verifyPassword, codeHash, randomId, newLedger, grantNewUserBonus, publicUser, adminPublicUser,
