const { redisConfig, readUsers, readOrders, readWithdrawals } = require('../maoyang-store.js');
const { currentUser, publicUser, normalizeEmail } = require('../maoyang-auth.js');
...
function publicOrder(order){
  ...
  return {
    ...
    couponDeduction:Number(order.couponDeduction||0), walletDeduction:Number(order.walletDeduction||0), finalAmount:Number(order.finalAmount||0), finalUsdt:Number(order.finalUsdt||0),
