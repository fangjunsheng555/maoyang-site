...
async function pushOrderAndWriteCodes(redis, order, codes, limit = 199) {
  const commands = [['LPUSH', ORDERS_KEY, JSON.stringify(order)], ['LTRIM', ORDERS_KEY, '0', String(limit)], ['DEL', CODES_KEY]];
  if (Array.isArray(codes) && codes.length > 0) commands.push(['RPUSH', CODES_KEY, ...codes.map((code) => JSON.stringify(code))]);
  await redisPipeline(redis, commands);
  return true;
}
...
async function writeUsersCodes(redis, users, codes) {
  const commands = [['DEL', USERS_KEY], ['DEL', CODES_KEY]];
  if (users.length > 0) commands.push(['RPUSH', USERS_KEY, ...users.map((item) => JSON.stringify(item))]);
  if (codes.length > 0) commands.push(['RPUSH', CODES_KEY, ...codes.map((item) => JSON.stringify(item))]);
  await redisPipeline(redis, commands);
  return true;
...
module.exports = {
  ...
  pushOrder, pushOrderAndWriteUsers, pushOrderAndWriteCodes, writeUsersCodesOrders, writeUsersCodes, writeUsersWithdrawals
};
